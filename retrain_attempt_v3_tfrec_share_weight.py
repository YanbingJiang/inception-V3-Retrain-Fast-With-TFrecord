...
###### UTLIZE SHARING WIGHTS TO OVERCOME TF.CASE DRAWBACK  ##############
'''
...
def weight_variable(shape):
  ...
  layer_weights = tf.get_variable('final_weights',initializer= initial )
...
def bias_variable(shape):
  ...
  layer_biases = tf.get_variable('final_biases', initializer=initial)
...
def nn_layer(input_tensor, input_dim, output_dim, layer_name):
  ...
  with tf.name_scope(layer_name):
    ...
    with tf.name_scope('Wx_plus_b'):
      logits = tf.matmul(input_tensor, layer_weights) + layer_biases
...
def add_final_val_ops(class_count, final_tensor_name, bottleneck_tensor, bottleneck_input, ground_truth_input):  

  layer_name = 'final_training_ops_val'  
  with tf.variable_scope("model", reuse=True):
    logits_val = nn_layer(bottleneck_input, BOTTLENECK_TENSOR_SIZE, class_count, layer_name)

  final_tensor_val =  tf.nn.softmax(logits_val, name=final_tensor_name +'_val')
  tf.summary.histogram('activations_val', final_tensor_val)  
  
  return (bottleneck_input, ground_truth_input, final_tensor_val)  
...
def main(_):  
  ...
  '''
  Since tf.case run every case then throw cases that not used, which waste many resources, a sharing weight architecture is built to solve this problem
  '''
  ...
  (train_step, cross_entropy, bottleneck_input, ground_truth_input, final_tensor) = add_final_training_ops(len(image_lists.keys()), 
    FLAGS.final_tensor_name, bottleneck_tensor, train_bottlenecks, train_ground_truth)

  # Add the sharing weight part for interval validation purpose
  (bottleneck_input_val, ground_truth_input_val, final_tensor_val) = add_final_val_ops(len(image_lists.keys()), 
    FLAGS.final_tensor_name, bottleneck_tensor, validation_bottlenecks, validation_ground_truth)

  (bottleneck_input_test, ground_truth_input_test, final_tensor_test) = add_final_val_ops(len(image_lists.keys()), 
    FLAGS.final_tensor_name, bottleneck_tensor, test_bottlenecks, test_ground_truth)
  
  # Create the operations we need to evaluate the accuracy of our new layer
  evaluation_step, prediction = add_evaluation_step(final_tensor, ground_truth_input)
  evaluation_step_val, prediction_val = add_evaluation_step(final_tensor_val, ground_truth_input_val)  
  evaluation_step_test, prediction_test = add_evaluation_step(final_tensor_test, ground_truth_input_test)  
  ...
  for i in range(FLAGS.how_many_training_steps):  
    ...
    if (i % FLAGS.eval_step_interval) == 0 or is_last_step:  
      ...
      validation_summary, validation_accuracy = sess.run([merged, evaluation_step_val])  
  ...
  # test_bottlenecks_value, test_ground_truth_value = sess.run([test_bottlenecks, test_ground_truth])
  test_accuracy, predictions = sess.run([evaluation_step_test, prediction_test])
